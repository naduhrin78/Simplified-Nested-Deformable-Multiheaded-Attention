...
class LunaTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, p):
        ...
        packed_context = (
            self.packed_context_layer_norm(
                (packed_context + p).reshape(b, c, -1).transpose(-2, -1).contiguous()
            )
            .transpose(-2, -1)
            .contiguous()
            .reshape(b, c, h, w)
        ...
        unpacked_context = (
            self.unpacked_context_layer_norm(
                (unpacked_context + x).reshape(b, c, -1).transpose(-2, -1).contiguous()
            )
            .transpose(-2, -1)
            .contiguous()
            .reshape(b, c, h, w)
...
class res_bottleneck(nn.Module):
    def __init__(self, in_ch, num_heads, expansion_factor=2.21):
        ...
        self.res_block1 = LunaTransformerEncoderLayer(
            in_ch, num_heads, expansion_factor
        )
        self.res_block2 = LunaTransformerEncoderLayer(
            in_ch, num_heads, expansion_factor
        )
        self.res_block3 = LunaTransformerEncoderLayer(
            in_ch, num_heads, expansion_factor
        )
        self.res_block4 = LunaTransformerEncoderLayer(
            in_ch, num_heads, expansion_factor
        )
        self.res_block5 = LunaTransformerEncoderLayer(
            in_ch, num_heads, expansion_factor
        )
    ...
    def forward(self, x):
        ...
        res5, res1_p5 = self.res_block5(
            res4 + res3 + res2 + res1, res1_p4 + res1_p3 + res1_p2 + res1_p1
        )
...
class Luna_Net(nn.Module):
    def __init__(self, in_channels, out_channels, factor):
        ...
        self.Conv1 = GatedConv2dWithActivation(
            in_channels, 64 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Conv2 = GatedConv2dWithActivation(
            64 // factor, 128 // factor, kernel_size=3, stride=2, padding=1
        )
        self.Conv3 = GatedConv2dWithActivation(
            128 // factor, 256 // factor, kernel_size=3, stride=2, padding=1
        )
        self.Conv4 = GatedConv2dWithActivation(
            256 // factor, 512 // factor, kernel_size=3, stride=2, padding=1
        )
        self.Conv5 = GatedConv2dWithActivation(
            512 // factor, 1024 // factor, kernel_size=3, stride=2, padding=1
        )

        self.dil_conv1 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.dil_conv2 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.dil_conv3 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.dil_conv4 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Up5 = GatedDeConv2dWithActivation(
            2, 1024 // factor, 512 // factor, kernel_size=3, padding=1
        )
        self.Up_conv5 = GatedConv2dWithActivation(
            1024 // factor, 512 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Up4 = GatedDeConv2dWithActivation(
            2, 512 // factor, 256 // factor, kernel_size=3, padding=1
        )
        self.Up_conv4 = GatedConv2dWithActivation(
            512 // factor, 256 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Up3 = GatedDeConv2dWithActivation(
            2, 256 // factor, 128 // factor, kernel_size=3, padding=1
        )
        self.Up_conv3 = GatedConv2dWithActivation(
            256 // factor, 128 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Up2 = GatedDeConv2dWithActivation(
            2, 128 // factor, 64 // factor, kernel_size=3, padding=1
        )
        self.Up_conv2 = GatedConv2dWithActivation(
            64 // factor, 64 // factor, kernel_size=3, stride=1, padding=1
        )

        self.Conv_1x1 = nn.Conv2d(64 // factor, out_channels, 1)

        self.p2_Conv1 = GatedConv2dWithActivation(
            in_channels, 64 // factor, kernel_size=3, stride=1, padding=1
        )

        self.p2_Conv2 = GatedConv2dWithActivation(
            64 // factor, 128 // factor, kernel_size=3, stride=2, padding=1
        )
        self.p2_Conv3 = GatedConv2dWithActivation(
            128 // factor, 256 // factor, kernel_size=3, stride=2, padding=1
        )
        self.p2_Conv4 = GatedConv2dWithActivation(
            256 // factor, 512 // factor, kernel_size=3, stride=2, padding=1
        )
        self.p2_Conv5 = GatedConv2dWithActivation(
            512 // factor, 1024 // factor, kernel_size=3, stride=2, padding=1
        )

        self.p2_dil_conv1 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.p2_dil_conv2 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.p2_dil_conv3 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )
        self.p2_dil_conv4 = GatedConv2dWithActivation(
            1024 // factor, 1024 // factor, kernel_size=3, stride=1, padding=1
        )

        self.p2_Up5 = GatedDeConv2dWithActivation(
            2, 1024 // factor, 512 // factor, kernel_size=3, padding=1
        )
        self.gmlp_attn1 = LunaTransformerEncoderLayer(512 // factor, 8, 2.33)
        self.p2_Up_conv5 = GatedConv2dWithActivation(
            1024 // factor, 512 // factor, kernel_size=3, stride=1, padding=1
        )

        self.p2_Up4 = GatedDeConv2dWithActivation(
            2, 512 // factor, 256 // factor, kernel_size=3, padding=1
        )
        self.gmlp_attn2 = LunaTransformerEncoderLayer(256 // factor, 8, 2.33)
        self.p2_Up_conv4 = GatedConv2dWithActivation(
            512 // factor, 256 // factor, kernel_size=3, stride=1, padding=1
        )

        self.p2_Up3 = GatedDeConv2dWithActivation(
            2, 256 // factor, 128 // factor, kernel_size=3, padding=1
        )
        self.gmlp_attn3 = LunaTransformerEncoderLayer(128 // factor, 8, 2.33)
        self.p2_Up_conv3 = GatedConv2dWithActivation(
            256 // factor, 128 // factor, kernel_size=3, stride=1, padding=1
        )

        self.p2_Up2 = GatedDeConv2dWithActivation(
            2, 128 // factor, 64 // factor, kernel_size=3, padding=1
        )
        self.gmlp_attn4 = LunaTransformerEncoderLayer(64 // factor, 8, 2.33)
        self.p2_Up_conv2 = GatedConv2dWithActivation(
            128 // factor, 64 // factor, kernel_size=3, stride=1, padding=1
        )
